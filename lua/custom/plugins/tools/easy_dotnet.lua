...
return {
  ...
  config = function()
    ...
    -- Run & Build
    map("<leader>dr",  function() dotnet.run() end,              "[D]otnet [R]un")
    map("<leader>dw",  function() dotnet.run_with_args() end,    "[D]otnet Run [W]ith Args")
    map("<leader>db",  function() dotnet.build() end,            "[D]otnet [B]uild")
    map("<leader>dB",  function() dotnet.build_solution() end,   "[D]otnet [B]uild Solution")
    map("<leader>dC",  function() dotnet.clean() end,            "[D]otnet [C]lean")
    map("<leader>do",  function() dotnet.restore() end,          "[D]otnet Rest[o]re")

    -- Watch Run (Hot Reload) — replaces old <C-F5> ToggleTerm workaround
    map("<C-F5>",      function() dotnet.run({ watch = true }) end, "Dotnet Watch Run (Hot Reload)")

    -- Test Runner (replaces neotest-dotnet keymaps)
    map("<leader>nt",  function() dotnet.test() end,             "[N]eotest: Run nearest [T]est")
    map("<leader>nf",  function() dotnet.test_solution() end,    "[N]eotest: Run [F]ile / Solution tests")
    map("<leader>ns",  function() dotnet.open_testrunner() end,  "[N]eotest: Toggle test [S]ummary panel")

    -- NuGet / Packages
    map("<leader>dn",  function() dotnet.nuget_search() end,     "[D]otnet [N]uGet search")
    map("<leader>da",  function() dotnet.add() end,              "[D]otnet [A]dd package")

    -- Secrets & config
    map("<leader>ds",  function() dotnet.user_secrets() end,     "[D]otnet [S]ecrets")

    -- Workspace diagnostics (full solution errors)
    map("<leader>dD",  function() dotnet.workspace_diagnostics() end, "[D]otnet Workspace [D]iagnostics")

    -- Entity Framework
    map("<leader>de",  function() dotnet.entity_framework() end, "[D]otnet [E]ntityFramework")

    -- Outdated packages (virtual text)
    map("<leader>du",  function() dotnet.outdated() end,         "[D]otnet Outdated ([U]pdate check)")

    -- Solution / project management
    map("<leader>dp",  function() dotnet.project() end,          "[D]otnet [P]roject view")

    -- New project/file templates
    map("<leader>dN",  function() dotnet.new() end,              "[D]otnet [N]ew template")
  end,
}
